(function(exports, global) {
    ...
    function _aes_asm(stdlib, foreign, buffer) {
        ...
        function gcm_decrypt(offset, length, g0, g1, g2, g3, g4, g5, g6, g7, g8, g9, gA, gB, gCDEF) {
            ...
            if ((length | 0) > 0) {
                s0 = HEAP[offset | 0], s1 = (length | 0) > 1 ? HEAP[offset | 1] : 0, s2 = (length | 0) > 2 ? HEAP[offset | 2] : 0, 
                s3 = (length | 0) > 3 ? HEAP[offset | 3] : 0, s4 = (length | 0) > 4 ? HEAP[offset | 4] : 0, 
                s5 = (length | 0) > 5 ? HEAP[offset | 5] : 0, s6 = (length | 0) > 6 ? HEAP[offset | 6] : 0, 
                s7 = (length | 0) > 7 ? HEAP[offset | 7] : 0, s8 = (length | 0) > 8 ? HEAP[offset | 8] : 0, 
                s9 = (length | 0) > 9 ? HEAP[offset | 9] : 0, sA = (length | 0) > 10 ? HEAP[offset | 10] : 0, 
                sB = (length | 0) > 11 ? HEAP[offset | 11] : 0, sC = (length | 0) > 12 ? HEAP[offset | 12] : 0, 
                sD = (length | 0) > 13 ? HEAP[offset | 13] : 0, sE = (length | 0) > 14 ? HEAP[offset | 14] : 0;
                sF = 0;
                _gcm_mult(Z0 ^ (s0 << 24 | s1 << 16 | s2 << 8 | s3), Z1 ^ (s4 << 24 | s5 << 16 | s6 << 8 | s7), Z2 ^ (s8 << 24 | s9 << 16 | sA << 8 | sB), Z3 ^ (sC << 24 | sD << 16 | sE << 8 | sF));
                _encrypt(g0, g1, g2, g3, g4, g5, g6, g7, g8, g9, gA, gB, gCDEF >>> 24, gCDEF >>> 16 & 255, gCDEF >>> 8 & 255, gCDEF & 255);
                HEAP[offset] = s0 ^ S0;
    ...
    function _gcm_ghash(data) {
        var asm = this.asm, heap = this.heap, dpos = data.byteOffset || 0, dlen = data.byteLength || data.length || 0, hpos = _aes_heap_start, hlen = 0, wlen = 0;
        while (dlen > 0) {
            wlen = _aes_heap_write(heap, hpos + hlen, data, dpos, dlen), hlen += wlen, dpos += wlen, 
            dlen -= wlen;
    ...
    function gcm_aes_reset(options) {
        ...
        if (adata !== undefined && adata !== null) {
            ...
            if (adata.byteLength === 0 || adata.byteLength > _gcm_data_maxLength) throw new IllegalArgumentError("illegal adata length");
    ...
    function gcm_aes_encrypt_process(data) {
        if (!this.key) throw new IllegalStateError("no key is associated with the instance");
        var asm = this.asm, heap = this.heap, iv = this.iv, counter = this.counter, dpos = data.byteOffset || 0, dlen = data.byteLength || data.length || 0, hpos = this.pos, hlen = this.len, rpos = 0, rlen = _aes_block_size * Math.floor((hlen + dlen) / _aes_block_size), wlen = 0;
        var result = new Uint8Array(rlen);
    ...
    function gcm_aes_encrypt_finish() {
        ...
        var alen = adata !== null ? adata.byteLength || adata.length || 0 : 0, clen = (counter - 1 << 4) + wlen;
    ...
    function gcm_aes_decrypt_process(data) {
        if (!this.key) throw new IllegalStateError("no key is associated with the instance");
        var asm = this.asm, heap = this.heap, iv = this.iv, counter = this.counter, tagSize = this.tagSize, dpos = data.byteOffset || 0, dlen = data.byteLength || data.length || 0, hpos = this.pos, hlen = this.len, rpos = 0, rlen = _aes_block_size * Math.floor((hlen + dlen - tagSize) / _aes_block_size), wlen = 0;
        var result = new Uint8Array(rlen);
    ...
    function hmac_sha1_bytes(data, password) {
        ...
        return get_hmac_sha1_instance().reset({
            password: password
    ...
    function pbkdf2_hmac_sha1_bytes(password, salt, iterations, dklen) {
        ...
        return get_pbkdf2_hmac_sha1_instance().reset({
            password: password
    ...
    function pbkdf2_hmac_sha256_bytes(password, salt, iterations, dklen) {
        if (password === undefined) throw new SyntaxError("password required");
        if (salt === undefined) throw new SyntaxError("salt required");
        return get_pbkdf2_hmac_sha256_instance().reset({
            password: password
        }).generate(salt, iterations, dklen).result;
    }
    ...
    function pbkdf2_hmac_sha512_bytes(password, salt, iterations, dklen) {
        if (password === undefined) throw new SyntaxError("password required");
        if (salt === undefined) throw new SyntaxError("salt required");
        return get_pbkdf2_hmac_sha512_instance().reset({
            password: password
        }).generate(salt, iterations, dklen).result;
    }
